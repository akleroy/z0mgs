...
; &%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%&%
  ...
  for ii = 0, ct_overlap-1 do begin
     ...
     this_index = index[ind[ii]]
     in_dir = strcompress(this_index.tile_dir, /rem)
; The image file
; ... specify either background subtracted or not
     if keyword_set(useint) then begin
        this_image_fname = $
           in_dir + strcompress(this_index.int_file, /rem)
     endif else begin
        this_image_fname = $
           in_dir + strcompress(this_index.bgsub_file, /rem)
     endelse
          
;    THE RELATIVE RESPONSE
     this_rrhr_fname = $
        in_dir + strcompress(this_index.rrhr_file, /rem)

;    THE FLAG FILE
     this_flag_fname = $
        in_dir + strcompress(this_index.flag_file,/rem)
